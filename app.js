...
medSchema.plugin(passportlocalMongoose);
medSchema.plugin(findOrCreate);
...
docSchema.plugin(findOrCreate); // end
...
passport.use(PostMed.createStrategy());
passport.use(PostDoc.createStrategy());
...
passport.serializeUser(function(user, done) {
  ...
  passport.deserializeUser(function(id, done) {
    PostMed.findById(id, function(err, user) {
      done(err, user);
    });
  });
  passport.deserializeUser(function(id, done) {
    PostDoc.findById(id, function(err, user) {
      done(err, user);
    });
  });
   // google part
  ...
  function(accessToken, refreshToken, profile, cb) {
      ...
      //  console.log(name);//my useful data
    Post.findOrCreate({ googleId: profile.id }, function (err, user) {
      return cb(err, user);
    });
    PostDoc.findOrCreate({ googleId: profile.id }, function (err, user) {
      return cb(err, user);
    });
    PostMed.findOrCreate({ googleId: profile.id }, function (err, user) {
      return cb(err, user);
    });
  }
)); 
...
app.get("/mymedicine",function(req,res){
  PostMed.find().sort({"problem":1});
  PostMed.find({googleId: req.user.googleId}, function(err, posts){
  ...
  PostMed.aggregate([
    {
      ...
      {
        "search": {
          "query":search, "path":["problem","symptom","medicine"]
        }
      }
    }
  ]).exec((err,posts)=>{
...
app.get("/mydoctor",function(req,res){
  PostDoc.find({googleId: req.user.googleId},function(err,posts){
    res.render("mydoctor",{
      posts:posts
    });
  });
